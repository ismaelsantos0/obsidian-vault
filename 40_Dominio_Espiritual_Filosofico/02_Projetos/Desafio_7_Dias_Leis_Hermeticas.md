# DESAFIO 7 DIAS — APLICANDO AS 7 LEIS HERMÉTICAS NA VIDA REAL

## AS 7 REGRAS DO DESAFIO HERMÉTICO

**1. Regra do Silêncio**
Você não comenta esse desafio com ninguém até terminar. Energia compartilhada antes da hora se dispersa. O que é interno se fortalece no silêncio.

**2. Regra da Execução Imediata**
Leu a prática do dia, executa no mesmo momento. Sem adiar, sem “depois eu faço”. Conhecimento não aplicado vira ilusão de progresso.

**3. Regra da Observação Brutal**
Durante os 7 dias, você não mente para si mesmo. Nem suaviza, nem justifica. Você observa quem você realmente é, não quem gostaria de ser.

**4. Regra da Não-Reação**
Sentiu um gatilho emocional, você pausa antes de reagir. Quem reage automaticamente é controlado. Quem observa, assume o controle.

**5. Regra da Responsabilidade Total**
Nada externo é culpa durante esses 7 dias. Tudo o que acontece é assumido como reflexo ou consequência. Sem exceções.

**6. Regra da Constância Invisível**
Você executa mesmo sem motivação, sem validação e sem ninguém vendo. O poder real é construído no que ninguém presencia.

**7. Regra do Compromisso Final**
Você vai até o fim, mesmo que perceba desconfortos. Porque depois que você vê, não tem como deixar de ver.

🔗 **Referência:** [Vídeo do YouTube](https://youtu.be/e3nO_IjTjLs)

> [!info] Como usar este desafio
> - **1 lei por dia**
> - **1 ação consciente**
> - **1 reflexão no final do dia**
> - Sem pular. Sem adaptar. Executa.

---

## Visão Geral (7 Dias)

| Dia | Lei | Foco do dia | Status |
| --- | --- | --- | --- |
| 1 | Mentalismo 🧠 | Vigilância e substituição de pensamentos | - [ ] Feito |
| 2 | Correspondência 🔁 | Organizar fora para alinhar dentro | - [ ] Feito |
| 3 | Vibração ⚡ | Mudar estado em até 15 minutos | - [ ] Feito |
| 4 | Polaridade ⚖️ | Encontrar utilidade no contraste | - [ ] Feito |
| 5 | Ritmo 🌊 | Usar picos e baixas com inteligência | - [ ] Feito |
| 6 | Causa e Efeito 🎯 | Gerar causa mensurável | - [ ] Feito |
| 7 | Gênero ⚡🌙 | Equilibrar ação e intuição | - [ ] Feito |

---

<div style="page-break-after: always;"></div>

> [!tip] Sugestão de Impressão
> Use **Imprimir** (ou exporte em PDF) e leve para uma folha **A4**.
> - No computador: `Menu do Obsidian → Exportar → PDF`
> - Ou: `Ctrl/Cmd + P` para imprimir

# 📝 Ficha Prática para Preencher (A4)

**Nome:** __________________________________________________
**Data de início:** ____ / ____ / ________
**Compromisso (7 dias):** __________________________________

---

## DIA 1 — Mentalismo 🧠
**“O todo é mente.”**

**Explicação prática (o que fazer):**
- A sua realidade externa nasce na sua paisagem interior. Durante o dia, anote 3 pensamentos automáticos negativos assim que surgirem.
- Para cada um, escreva uma substituição consciente que seja verdadeira e útil para agir melhor. Mude o padrão, mude o fato.
- Faça uma janela de 2 horas de vigilância mental ativa, observando e substituindo na hora.

**Prática (pensamento negativo → substituição):**
- Pensamento 1: __________________________ → __________________________
- Pensamento 2: __________________________ → __________________________
- Pensamento 3: __________________________ → __________________________

**Desafio (2h de vigilância mental ativa):**
- Horário (início): ___________
- Horário (fim): ___________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 2 — Correspondência 🔁
**“O que está dentro está fora.”**

**Explicação prática (o que fazer):**
- O seu ambiente é um espelho da sua mente. Escolha 1 área externa específica que está bagunçada (mesa, quarto, agenda, arquivos do celular).
- Observe qual padrão interno ela espelha (procrastinação, excesso, confusão, falta de prioridade).
- Organize essa área em 20 a 40 minutos, com começo e fim claros. A ordem externa ajudará a induzir a ordem interna.
- Finalize anotando como o “fora” mudou a sensação do “dentro”.

**Área bagunçada que vou observar:** _______________________________________
**Como isso reflete dentro de mim:** ________________________________________
**Algo externo que vou organizar:** __________________________________________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 3 — Vibração ⚡
**“Nada está parado.”**

**Explicação prática (o que fazer):**
- Tudo é energia em movimento. Assim que perceber queda de energia, ansiedade ou irritação, você vai escolher 1 ação para mudar sua sintonia.
- O objetivo é elevar a frequência e sair do estado ruim em até 15 minutos, sem negociar com a mente.
- Registre o gatilho (o que baixou sua vibração) e o que funcionou melhor.

**Escolha 1 para elevar a vibração:**
- [ ] Treinar
- [ ] Ouvir música que eleva
- [ ] Fazer algo que dá energia
- [ ] Outro: ______________________________________________________________

**Desafio (mudar estado em até 15 minutos):**
- O que aconteceu: _______________________________________________________
- O que fiz para mudar: ___________________________________________________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 4 — Polaridade ⚖️
**“Tudo tem dois polos.”**

**Explicação prática (o que fazer):**
- Todo sentimento tem seu oposto na mesma escala. Escolha 1 fato negativo real do dia (sem romantizar).
- Encontre uma utilidade prática: o que isso está mostrando, pedindo ou corrigindo. Busque o caminho do meio.
- Transforme em 1 aprendizado que vire ação (transmutação) para amanhã.

**Fato negativo de hoje:** _________________________________________________
**Utilidade (lado positivo sem romantizar):** _______________________________
**Aprendizado prático:** ___________________________________________________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 5 — Ritmo 🌊
**“Tudo tem fluxo e refluxo.”**

**Explicação prática (o que fazer):**
- A vida é cíclica. Identifique seu pico e sua baixa de energia de hoje, com horário e estado.
- Use o pico para produzir algo concreto (aproveite a maré alta).
- Use a baixa para recuperação intencional (sem culpa), com uma estratégia definida (neutralização).

**Meu pico de energia (horário/estado):** ___________________________________
**Minha baixa de energia (horário/estado):** ________________________________

**Plano do dia:**
- Pico → o que vou produzir: _____________________________________________
- Baixa → como vou recuperar: ___________________________________________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 6 — Causa e Efeito 🎯
**“Nada acontece por acaso.”**

**Explicação prática (o que fazer):**
- Nós somos os causadores do nosso sucesso ou fracasso. Defina 1 resultado que você quer e que faria diferença real.
- Escreva a causa que realmente produz esse efeito (não um desejo passivo, mas uma causa real).
- Escolha 1 ação direta, mensurável e com horário marcado para executar hoje (seja a causa, não o efeito).

**Resultado que eu quero:** ________________________________________________
**Causa real que gera esse efeito:** ________________________________________
**1 ação direta e mensurável:** _____________________________________________

**Reflexão do dia:**
_________________________________________________________________________

---

## DIA 7 — Gênero ⚡🌙
**“Tudo tem energia masculina e feminina.”**

**Explicação prática (o que fazer):**
- O equilíbrio destas energias evita a exaustão e a estagnação. Masculino: escolha 1 ação importante e objetiva e execute (direção, decisão, corte de excesso).
- Feminino: antes de agir em algo sensível, faça uma pausa curta para intuir, acolher e perceber nuances.
- Tome 1 decisão unindo razão (dados, força Yang) e intuição (sinais internos, sabedoria Yin).

**Ação importante que vou executar (masculino):** __________________________
**Pausa e reflexão antes de agir (feminino):** ______________________________
**Decisão importante (razão + intuição):** __________________________________

**Reflexão final (7 dias):**
_________________________________________________________________________
